...
curl_setopt_array($curl, array(
    CURLOPT_URL => base_url() . '/Register/getuser/' . $_GET['id'],
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" name="fname" value="<?php echo $data->fname; ?>" class="form-control" id="First Name" aria-describedby="fname" placeholder="Enter First Name" required>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="lname">Last Name</label>
                            <input type="text" name="lname" value="<?php echo $data->lname; ?>" class="form-control" id="lname" placeholder="Last Name" required>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="phoneNumber">Phone Number</label>
                            <input type="number" name="phoneNumber" value="<?php echo $data->phoneNumber; ?>" class="form-control" id="phoneNumber" placeholder="Phone Number" required>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control" value="<?php echo $data->division; ?>" name="division" id="division" onchange="UpdateWorker()">
                                <option value="">Select</option>
                                <?php
                                foreach ($getAllDivisions as $divisions) {
                                ?>
                                    <option value="<?php echo $divisions->division; ?>" <?php echo $divisions->division === $data->division ? 'selected' : ''; ?>>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control" value="<?php echo $data->subDivision; ?>" name="subDivision" id="subDivision" onchange="getAllSections(subDivision.value)">
                                <option value="">Select</option>
                            </select>
                        </div>
                    </div>
...
<script>
    function UpdateWorker() {
        ...
        xhr.onload = function() {
            ...
            $data.forEach((element, i) => {
                ...
                if (element.subDivision === <?php echo json_encode($data->subDivision); ?>) {
                    opt.selected = true;
                }
    ...
    function getAllSections(value) {
        ...
        xhr.onload = function() {
            ...
            $data.forEach((element, i) => {
                ...
                if (element.section === <?php echo json_encode($data->section); ?>) {
                    opt.selected = true;
                }
    ...
    UpdateWorker();
    getAllSections('<?php echo $data->subDivision; ?>');
</script>
...
<script>
    document.querySelector("#updateForm").addEventListener("submit", function(e) {
        ...
        if (password === confPassword) {
            const xhr = new XMLHttpRequest();
            const formElements = document.forms.updateForm;
            ...
            xhr.open('POST', '<?php echo site_url('Register/updateuser/' . $_GET['id']); ?>');
            xhr.send(formData);
        } else {
